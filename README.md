...
x
n
 efficiently using divide-and-conquer recursion (fast power).
...
n
:
Output current permutation of string 
s
Return
For 
j
...
Graphs
Linear Search Time Complexity Graph
...
Graphs
Selection Sort Time Complexity Graph
...
/
2
Compute actual sum of all elements in 
a
r
r
Return 
...
Graphs
Insertion Sort Time Complexity Graph
...
Graphs
Binary Search Time Complexity Graph
...
>
 pivot on right
Recursively apply on left and right partitions
Time Complexity (TC)
Best/Average: 
O
(
...
Graphs
Merge Sort Time Complexity Graph
...
Graphs
Iterative Quick Sort Time Complexity Graph
...
Cargo loading
Graphs
Fractional Knapsack Time Complexity Graph
...
Find the lowest point as pivot
...
Stack contains the convex hull vertices
Time Complexity (TC)
O
(
n
...
Graphs
Convex Hull Time Complexity Graph
...
Graphs
K-th Smallest Time Complexity Graph
...
=
0
Use a min-priority queue to pick node with minimum distance
For each adjacent vertex, update distance if shorter path found
...
Initialize all vertices as not in MST; set 
d
i
s
t
...
Select first activity; track last finish time
For each remaining activity:
If start time 
≥
 last finish time, select it and update last finish
...
Track the current recursion stack
If a visited vertex is found in the current stack, a cycle exists
Time Complexity (TC)
O
(
V
...
Graphs
Kruskal's MST Time Complexity Graph
...
0
, all others infinity
For each stage layer from source:
For each vertex 
u
 in current stage:
Relax edges to all vertices 
v
 in next stage
Track parent array to reconstruct path
Time Complexity (TC)
O
(
V
...
0
, all others infinity
Traverse stages backward from destination:
...
Graphs
Matrix Chain Multiplication Time Complexity Graph
...
Graphs
Floyd-Warshall All-Pairs Shortest Path Graph
...
0
, all others infinity
Iterate over all subsets; for each state update reachable cities
Return minimum of 
d
p
[
...
Graphs
N-Queens Time Complexity Graph
...
Recurse if remaining elements can still reach 
W
Time Complexity (TC)
...
Graphs
Sum of Subsets Time Complexity Graph
...
Check if current color assignment is safe (no adjacent vertex has same color)
If safe, recurse for next vertex
...
Start from vertex 0
Recursively add adjacent unvisited vertices to path
If all vertices visited, check if last vertex connects back to first
If yes, Hamiltonian cycle found; else backtrack
Time Complexity (TC)
O
(
n
